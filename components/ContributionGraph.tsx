...
export default function ContributionGraph() {
  ...
  return (
    <div className="w-full flex flex-col lg:flex-row gap-6 items-start">
      {/* Left: Calendar Box + Activity Timeline */}
      ...
      {/* Right: Vertical Year Filters */}
      <div className="flex flex-col gap-1 w-full lg:w-32 self-start">
        {years.map((y) => (
          <button
            key={y}
            onClick={() => setYear(y)}
            className={`px-4 py-2 text-sm rounded-md w-full text-left transition-all ${
              year === y 
                ? "bg-[#1f6feb] text-white font-medium shadow-sm" 
                : "bg-transparent text-[#7d8590] hover:text-[#e6edf3] hover:bg-[#21262d]/50"
            }`}
